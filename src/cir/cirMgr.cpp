...
bool CirMgr::cmp(CirGate *&a, CirGate *&b)
{
   bool c = (a->id()) < (b->id());
   return c;
}
bool CirMgr::cmp1(GateList &a, GateList &b)
{
...
void CirMgr::printdfs(vector<CirGate *> gatelist, bool reset) const
{
   static int index;
   if (reset == true)
      index = 0;
...
void CirMgr::countdfs(CirGate *gate, int marked) const
{
...
void CirMgr::adfs(CirGate *gate, ostream &outfile, int marked) const
{
   if (gate->marked() == marked)
   {
      return;
...
void CirMgr::idfs(CirGate *gate, int marked) const
{
   if (gate->marked() == marked)
   {
      return;
